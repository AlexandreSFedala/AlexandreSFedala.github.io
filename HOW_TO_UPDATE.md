...
This guide provides simple, step-by-step instructions for adding new projects, skills, and certifications to your portfolio. All content is managed in the `data/` directory using JSON files.
...
All project information is stored in `data/projects.json`.
...
5.  **Paste and edit:** Paste the copied block at the end of the list (before the closing `]`) and edit the following fields. **Note that all keys and string values must be enclosed in double quotes.**
...
All skill information is stored in `data/skills.json`.

1.  **Open the file:** `data/skills.json`
2.  **Choose a category:** Decide if the skill is `"technical"` or `"soft"`.
3.  **Add the skill:** In both the `"en"` and `"fr"` sections, add a new object to the chosen category's list with the following format:
...
All certification information is stored in `data/certifications.json`.

1.  **Open the file:** `data/certifications.json`
2.  **Add the certification:** In both the `"en"` and `"fr"` sections, add a new object to the list with the following format:
    ...
    **Example:**
    ...
    "en": [
      ...
      { "img": "images/new-cert.png", "title": "New Certification Title" }
    ],
    ```
...
To make a project appear on the interactive map, you need to add a `"coordinates"` property to its object in the `data/projects.json` file.
